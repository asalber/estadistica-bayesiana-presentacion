...
"
tension_post <- stan(model_code = modelo, data = list(n = 200, k = 70), 
...
```{r}
library(knitr)
summary(tension_post, pars = "p", probs = c(0.05, 0.95))$summary |> 
  kable()
```
...
library(tidyverse)
library(bayesplot)
mcmc_intervals(tension_post, pars = "p", 
...
"
tuberculosis_post <- stan(model_code = modelo, 
    data = list(y = 48/3), 
    chains = 4, iter = 5000*2, seed = 123)
```
</details>
...
library(tidyverse)
library(bayesplot)
mcmc_intervals(tuberculosis_post, pars = "lambda", 
  prob = 0.5, prob_outer = 0.9, 
  point_est = "mean") + 
...
En el ejemplo de la incidencia de la tuberculosis, realizar el contraste para ver si la incidencia es mayor que 15 por cada 100.000 habitantes-año.

¿Cómo plantearías el contraste para ver si la incidencia es 18?
:::
...
En este caso la probabilidad de que la incidencia sea mayor que 15 es menor que 0.1, por lo que parece razonable rechazar la hipótesis nula y aceptar al hipótesis alternativa de que la incidencia de tuberculosis es menor o igual al 15% por cada 100.000 habitantes-año.
</details>
...
donde $f(\tilde x|\theta)$ es la función de verosimilitud de la nueva observación $\tilde x$ dado el parámetro $\theta$.
:::
...
tension_pred <- tension_post |>
  ...
  select(p, pred)
head(tension_pred, 5) |>  # Mostramos las 5 primeras predicciones.
  kable()
```
...
#| output-location: column
tension_pred |>
  ggplot(aes(x = pred, y = after_stat(prop))) +
